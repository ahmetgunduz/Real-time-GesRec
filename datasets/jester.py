...
def video_loader(video_dir_path, frame_indices, modality, sample_duration, image_loader):
    ...
    if modality == 'RGB':
        ...
        for i in frame_indices: # index 35 is used to change img to flow
            image_path_u = os.path.join('/usr/home/kop/datasets/UCF-101_flow',
                                        video_dir_path[35:], 'u',
                                        'image_{:05d}.jpg'.format(i))
            image_path_v = os.path.join('/usr/home/kop/datasets/UCF-101_flow',
                                        video_dir_path[35:], 'v',
                                        'image_{:05d}.jpg'.format(i))
            image_u = image_loader(image_path_u, modality)
            image_v = image_loader(image_path_v, modality)

            if os.path.exists(image_path_u):
                video.append(image_u)
                video.append(image_v)
            else:
                return video
        ...
        for i in frame_indices: # index 35 is used to change img to flow
            ...
            image_path_u = os.path.join('/data2/jester/flow/u',
                                        video_dir_path[18:],
                                        '{:05d}.jpg'.format(i-1))
            image_path_v = os.path.join('/data2/jester/flow/v',
                                        video_dir_path[18:],
                                        '{:05d}.jpg'.format(i-1))
            if not os.path.exists(image_path_u):
                image_path_u = os.path.join('/data2/jester/flow/u',
                                            video_dir_path[18:],
                                            '{:05d}.jpg'.format(i))
                image_path_v = os.path.join('/data2/jester/flow/v',
            ...
            image_u = image_loader(image_path_u, 'Flow')
            image_v = image_loader(image_path_v, 'Flow')
        ...
        for i in frame_indices: # index 35 is used to change img to flow
            image_path_first  = os.path.join(video_dir_path, '{:05d}.jpg'.format(i))
            image_path_second = os.path.join(video_dir_path, '{:05d}.jpg'.format(i-1))

            if not os.path.exists(image_path_second):
                image_path_second = os.path.join(video_dir_path, '{:05d}.jpg'.format(i))
        ...
        for i in frame_indices: # index 35 is used to change img to flow
            image_path_first  = os.path.join(video_dir_path, '{:05d}.jpg'.format(i))
            image_path_second = os.path.join(video_dir_path, '{:05d}.jpg'.format(max(1,i-4)))
            #image_path_third = os.path.join(video_dir_path, '{:05d}.jpg'.format(max(1,i-2)))
            #image_path_fourth = os.path.join(video_dir_path, '{:05d}.jpg'.format(max(1,i-3)))
